...
export function Navigation() {
  ...
  const handleDownloadResume = () => {
    ...
    alert('Resume download will be implemented with actual PDF file');
  };

  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-[#0A1628]/90 backdrop-blur-lg border-b border-[#00D9FF]/10' : 'bg-transparent'
      }`}
    >
            ...
            {navLinks.map((link) => (
              <a
                ...
                className={`text-sm transition-colors hover:text-[#00D9FF] ${
                  activeSection === link.href.slice(1) ? 'text-[#00D9FF]' : 'text-gray-300'
                }`}
              >
                {link.name}
              </a>
